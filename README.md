...
본 프로젝트는 비대면 상황에서 사용자의 얼굴 표정을 인식하여 감정을
분류하고, 실시간으로 이모지 및 통계를 제공하는 시스템입니다. 카메라를
켜지 않은 상태에서도 감정을 전달할 수 있는 기능을 지원하여 원활한 소통을
돕습니다.
...
-   **출처**: Kaggle 공개 데이터셋 (\~7,000장) + 팀원 수집 데이터(50장)
-   **처리 방식**:
    -   랜드마크 좌표 추출 및 정규화
    -   Excel/CSV로 정리 후 모델 학습
